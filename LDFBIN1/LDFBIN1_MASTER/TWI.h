...
void TWI_init_slave(uint8_t slave_addr)
{
	...
	//TWCR =	(1<<TWEA)|(1<<TWEN);				// Enable ACK when addressed.
}
...
{
	while ((TWSR & 0xF8) != 0x60)				// Loop until correct acknoledgement has been recieved
	{
		TWCR = (1<<TWINT)|(1<<TWEN)|(1<<TWEA);	// Enable TWI, clear TWI interrupt flag
		while (!(TWCR &(1<<TWINT)));			// Wait for TWINT flag reciever
	}
}
...
{
	while ((TWSR & 0xF8) != 0xA8)				// Loop until correct acknoledgement has been recieved
	{
		TWCR = (1<<TWINT)|(1<<TWEN)|(1<<TWEA);	// Enable TWI, clear TWI interrupt flag
