...
public class BluetoothLeGatt extends Service {
    ...
    public final static UUID UUID_LIGHT_COMMAND =
            UUID.fromString(GattAttributes.LIGHT_COMMAND);
    public final static UUID UUID_LIGHT_STATUS =
            UUID.fromString(GattAttributes.LIGHT_STATUS);
    public final static UUID UUID_LIGHT_CONTROL_SERVICE =
            UUID.fromString(GattAttributes.LIGHT_CONTROL_SERVICE);
    ...
    private void broadcastUpdate(final String action,
        ...
        if (UUID_LIGHT_STATUS.equals(characteristic.getUuid())) {
        ...
        if (UUID_LIGHT_COMMAND.equals(characteristic.getUuid())) {
            final byte[] data = characteristic.getValue();
            if (data != null && data.length > 0) {
                final StringBuilder stringBuilder = new StringBuilder(data.length);
                for (byte byteChar : data)
